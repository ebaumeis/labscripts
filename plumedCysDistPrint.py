...
def printComments(output):
  output.write('# plumed input file to print the vectors defining the direction of the cysteines' + 2*'\n')
...
def printTorsionCarbons(gqds,output):
  ...
  for x in gqds:
    ...
    output.write('gra%dcysdist1: DISTANCE ATOMS=%d,%d COMPONENTS\n' % ((y+1),cList[0],cList[1]))
    output.write('gra%dcysdist2: DISTANCE ATOMS=%d,%d COMPONENTS\n' % ((y+1),cList[2],cList[3]))
    output.write('\n')
    y = y+1
...
def printPlumed(gqds,output):
  ...
  for x in range(1,len(gqds)+1):
    if x < len(gqds):
      output.write('gra%scysdist1.x,gra%scysdist1.y,gra%scysdist1.z,gra%scysdist2.x,gra%scysdist2.y,gra%scysdist2.z,' % (x,x,x,x,x,x))
    else:
      output.write('gra%scysdist1.x,gra%scysdist1.y,gra%scysdist1.z,gra%scysdist2.x,gra%scysdist2.y,gra%scysdist2.z' % (x,x,x,x,x,x))
  output.write(' FILE=cysteineVectors')
  output.write('\n')
